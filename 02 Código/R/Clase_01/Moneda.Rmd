...
## Supuestos:
Supongamos un proceso estocastico U_t, el cual es puramente aleatorio, en los siguientes terminos:
i)   $\mathbb{E}[U_t] = mu = 0$

ii)  $Var[U_t] = \mathbb{E}[(U_t - mu)^2] = \mathbb{E}[(U_t)^2] = sigma^2$

ii)  $Cov[U_t,U_s] = \mathbb{E}[(U_t - mu)(U_s - mu)] = \mathbb{E}[U_t U_s] = 0$, para todo t distinto de s

## Algunos códigos base
Utilizaremos una funcion de numeros aleatorios: 
...
```
Supongamos $\mathbb{E}[U_t] = 0$ y $Var[U_t] = 4$
...
## Generamos una función que hace un proceso de cualquier dimensión, para el caso del lanzamiento de una moneda y un proceso estocástico del tipo $X_t = X_t-1 + U_t$
